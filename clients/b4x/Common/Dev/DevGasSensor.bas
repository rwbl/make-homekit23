...
' Brief:		Getter / setter for the gas sensor device.
...
Public Sub SetEnabled(state As Boolean)
	Dim deviceid As Byte 	= BLEConstants.DEV_GAS_SENSOR
...
' Parses a 3-byte gas sensor BLE payload.
' Example motion detected: 0A0201 -> DeviceID=0x0A, Command=0x02, Value=1
Public Sub Parse(data() As Byte) As Map
	Log($"[DevGasSensor.Parse] data=${HMITileUtils.ByteConv.HexFromBytes(data)}"$)
	Dim result As Map

	result.Initialize
	If data.Length < 3 Then
		Log("[DevGasSensor.Parse][E] Data too short, expect 3 bytes.")
		Return result
	End If
