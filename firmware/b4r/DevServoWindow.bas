...
Private Sub Process_Globals
	...
	Private TIMER_SLOT As Int 		= 0		' 0–3 (ESP32 has 4 hardware PWM timers)
	Private OPEN_POS As UInt 		= 100
...
Public Sub Initialize(pinnr As Byte)
	...
	Log("[DevServoWindow.Initialize][I] OK, pin=", pinnr, ", timerslot=", TIMER_SLOT)
End Sub
...
Public Sub Set(action As Byte)
	Log("[DevServoWindow.Set][I] action=", action)
...
Public Sub ProcessMQTT(storeindex As Byte)
	...
	Log("[DevServoWindow.ProcessMQTT] storeindex=", storeindex, ", payload=", payload)
...
Private Sub ConfirmMQTT(action As Byte)
	Dim topic() As String = Array As String(MQTTTopics.TOPIC_SERVO_WINDOW_STATUS)
	Dim payload() As String

	If action = MQTTTopics.ACTION_OPEN_VAL Then
		payload = Array As String(MQTTTopics.PAYLOAD_SERVO_WINDOW_OPEN)
	Else
		payload = Array As String(MQTTTopics.PAYLOAD_SERVO_WINDOW_CLOSED)
	End If
	MQTTClient.Publish(topic, payload)
	Log("[DevServoWindow.ConfirmPosition][I] json=", payload(0))
...
'	Example: Set window open = 060101
'
' Get the state open or closed (Command 0x02)
' 	Length: 2 Bytes
' 	Byte 0 Device:	0x06 > Servo Window
' 	Byte 1 Command:	0x02 > Get
'	Example: Get window state = 0602
'
' Parameters:
'   storeindex - Index of the global store buffer.
Public Sub ProcessBLE(storeindex As Byte)
	Dim payload() As Byte = GlobalStoreHandler.GetSlot(storeindex)
	Log("[DevServoWindow.ProcessBLE] storeindex=", storeindex, ", payload=", Convert.BytesToHex(payload))
...
Public Sub WriteToBLE(command As Byte, state As Byte)
	Dim payload() As Byte = Array As Byte(CommBLE.DEV_SERVO_WINDOW, command, state)
